...
namespace Nave.VR
{
    internal abstract class TrackingEvnBase
    {
        ...
        private void CheckDeviceRemovedOrAdded()
        {
            ...
            TryCheckNodeState(NaveVR.headAnchor, XRNode.Head);

            TryCheckNodeState(NaveVR.leftHandAnchor, XRNode.LeftHand);

            TryCheckNodeState(NaveVR.rightHandAnchor, XRNode.RightHand);

            TryCheckTrackNodeState(NaveVR.pelivsAnchor, XRNode.HardwareTracker);

            TryCheckTrackNodeState(NaveVR.leftFootAnchor, XRNode.HardwareTracker);

            TryCheckTrackNodeState(NaveVR.rightFootAnchor, XRNode.HardwareTracker);
        }
        ...
        private void UpdateInputDeviceStates()
        {
            for (int i = xRNodeStates.Count - 1; i >= 0; i--) {
                ...
                if (nodeState.uniqueID == NaveVR.headAnchor.uniqueID) 
                    FillPoseMetadata(NaveVR.headAnchor, ref nodeState);

                else if (nodeState.uniqueID == NaveVR.pelivsAnchor.uniqueID) 
                    FillPoseMetadata(NaveVR.pelivsAnchor, ref nodeState);

                else if (nodeState.uniqueID == NaveVR.leftFootAnchor.uniqueID) 
                    FillPoseMetadata(NaveVR.leftFootAnchor, ref nodeState);

                else if (nodeState.uniqueID == NaveVR.rightFootAnchor.uniqueID) 
                    FillPoseMetadata(NaveVR.rightFootAnchor, ref nodeState);

                else if (nodeState.uniqueID == NaveVR.leftHandAnchor.uniqueID) 
                    FillMetadata(NaveVR.leftHandAnchor, ref nodeState);

                else if (nodeState.uniqueID == NaveVR.rightHandAnchor.uniqueID) 
                    FillMetadata(NaveVR.rightHandAnchor, ref nodeState);
            }
        }
        ...
        {
            XRNodeState xRNodeState = default(XRNodeState);
            var pelvis = NaveVR.pelivsAnchor;
            var lfoot = NaveVR.leftFootAnchor;
            var rfoot = NaveVR.rightFootAnchor;
        ...
        private void InputTracking_nodeAdded(TrackingAnchor anchor, ref XRNodeState xRNodeState)
        {
            ...
            NaveVR.OnTrackerConnected(anchor);
        }

        private void InputTracking_nodeRemoved(TrackingAnchor anchor)
        {
            if (anchor == null || !anchor.connected) return;
            NaveVR.OnTrackerDisconnected(anchor);
            anchor.Disconnect();
        }
    }
}
