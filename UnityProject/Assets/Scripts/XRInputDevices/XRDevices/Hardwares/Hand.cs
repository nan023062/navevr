...
namespace NaveXR.InputDevices
{
    ...
    public class Hand : Hardware
    {
        ...
        internal override void OnConnected(InputNode inputNode)
        {
            m_connected = true;
            isLeft = inputNode.type == NodeType.LeftHand;
            if (mNextDeviceName == inputNode.name || mDeviceName == inputNode.name)
            {
                OnRefreshVisiable();
                return;
            }
            if (mIsLoadingAsset)
            {
                mNextDeviceName = inputNode.name;
            }
            else
            {
                LoadHandResAsync(inputNode.name);
            }
        }
        ...
        private void OnInitController(GameObject go)
        {
            ...
            m_Controller = go.GetComponent<Controller>();
            if (m_Controller == null)
                m_Controller = go.AddComponent<Controller>();
        ...
        private Dictionary<XRKeyCode, ControlTip> m_KeyControlTips;
        ...
        public void ShowControlTips(string json_tips)
        {
            ...
            while (e.MoveNext())
            {
                ...
                {
                    m_KeyControlTips[XRKeyCode.Trigger].Show(true, e.Current.Value); 
                }
                else if (e.Current.Key == "grip")
                {
                    m_KeyControlTips[XRKeyCode.Grip].Show(true, e.Current.Value);
                }
            }
        }
